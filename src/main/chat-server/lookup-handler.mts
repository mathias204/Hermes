import type { IncomingMessage, LookupRequest, LookupResult, User } from '../protocol/proto.mjs';
 ...
 */
async function onLookupRequest(ws: IWebSocket, user: User, request: LookupRequest): Promise<void> {
  ...
  const messages = await new Query(Table.MESSAGES).results(request.channel_id);
 ...
 */
function getMessagesAroundIndex(
 ...
 * @returns void
 */
function handleLookupError(ws: IWebSocket, error_code: number, reason: string): void {
