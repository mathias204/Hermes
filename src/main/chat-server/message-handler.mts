 ...
 * @returns Promise<void>
 */
async function messageHistory(ws: IWebSocket, user: User, request: RequestMessageHistory): Promise<void> {
  ...
  if (!hasAccess) {
    ...
    let messagesDB = await new Query(Table.MESSAGES).results(request.channel_id);
    ...
    sendToClient(ws, { command: 'message_history_response', data: response });
 ...
 * @returns void
 */
function handleMessageSendingError(ws: IWebSocket, error_code: number, reason: string): void {
 ...
 * @returns void
 */
function handleMessageHistoryError(ws: IWebSocket, error_code: number, reason: string): void {
  debug(`Lookup error: ${error_code} - ${reason}`);
  sendToClient(ws, {
