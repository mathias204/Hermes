 ...
 */
export async function makeFriends(x: string, y: string): Promise<void> {
 ...
 * @returns Promise<void>
 */
export async function breakUpFriends(x: string, y: string): Promise<void> {
  await new Query(Table.USERS)
    .filter(({ email_ID }) => email_ID === x || email_ID === y)
    .update((entry) => {
      if (entry.email_ID === x) {
 ...
 */
export async function checkFriends(x: string, y: string): Promise<boolean> {
  const userX = await new Query(Table.USERS).filter(({ email_ID }) => email_ID === x).results();
 ...
 */
export async function loadMessages(channel: string, n: number): Promise<MessageEntry[]> {
  const result = await new Query(Table.MESSAGES).results(channel);
